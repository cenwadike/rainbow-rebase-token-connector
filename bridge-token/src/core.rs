...
// Use the highest value that fits in a uint128 for max granularity.
pub const TOTAL_GONS: Balance = Balance::MAX;

/// Implementation of a rebase FungibleToken.
/// Allows to include NEP-141 compatible rebase token to any contract.
/// Internal balance is recorded as gons.
/// publicly viewed balance is recorded as fragments.

#[derive(BorshDeserialize, BorshSerialize)]
pub struct FungibleToken {
    /// AccountID -> gons balance.
    pub accounts: LookupMap<AccountId, Balance>,

    /// Total supply of the token.
    pub total_supply: Balance,

    /// Gons that represent a fragment
...
impl FungibleToken {
    pub fn new<S>(prefix: S, global_ampl_supply: Balance) -> Self
    where
        S: IntoStorageKey,
    {
        let gons_per_fragment = TOTAL_GONS.checked_div(global_ampl_supply).unwrap();
        let mut this = Self {
    ...
    pub fn internal_transfer(
        ...
        let internal_amount = amount.checked_mul(self.gons_per_fragment).unwrap();
        self.internal_withdraw(sender_id, internal_amount);
        self.internal_deposit(receiver_id, internal_amount);
        FtTransfer {
...
impl FungibleTokenCore for FungibleToken {
    ...
    fn ft_balance_of(&self, account_id: AccountId) -> U128 {
        self.accounts
            .get(&account_id)
            .unwrap_or(0)
            .checked_div(self.gons_per_fragment)
            .unwrap()
            .into()
    }
}
...
impl FungibleToken {
    ...
    pub fn internal_ft_resolve_transfer(
        ...
        // transform fragments to gons
        let amount: Balance = amount.0.checked_mul(self.gons_per_fragment).unwrap().into();
